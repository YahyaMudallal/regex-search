...
entre mesures. Le protocole compare donc les usages en ligne de commande. Pour
comparer uniquement les algorithmes, il faudra un benchmark distinct dans une JVM
persistante. L'ordre aléatoire atténue un biais d'ordre, sans supprimer le bruit
...
Le [README principal](../README.md) présente le projet et le
[chapitre expérimental](../docs/05-experiences.md) analyse les mesures publiées.
Pour reproduire les dix expériences, avec 20 observations par moteur :

```bash
./scripts/report-campaign.sh target/rapport-reproduction --runs 20
```

Ce script compile une fois, crée les corpus dérivés sous `target/report-corpora/`,
puis lance les comparaisons en série. Il enregistre les empreintes des sources et
les paramètres de la campagne. Le dossier de résultats doit être nouveau.

`scripts/plot-report.py` produit ensuite les figures à partir des CSV, sans refaire
les mesures. Il nécessite Python ≥ 3.11 et la dépendance optionnelle définie dans
`requirements-report.txt`. Voir le [guide de reproduction](../docs/01-utilisation.md#5-reproduire-toute-la-campagne-du-rapport).
Les dépendances de tracé ne sont pas nécessaires pour `run.sh` ou les benchmarks.
